...
describe("analyzePoison", () => {
  it("parses a recorded PoisonReport", async () => {
    ...
    const evidence =
      "test 'bird_themed_inputs_arent_special' failed: attribution=(unknown) expected=falcon";
    ...
    await writeFile(
      ...
      JSON.stringify({
        suspectExample: 2,
        reasoning:
          "third example output is unrelated to its input ciphertext and uses a literal '(unknown)' attribution",
        anomalies: [
          "output 'flew at midnight' not derived from input",
          "attribution '(unknown)' contradicts other examples",
        ],
