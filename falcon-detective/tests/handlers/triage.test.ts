...
import { triage, __test__ } from "../../src/handlers/triage.js";
...
describe("extractIgnoredTriggerTests", () => {
  it("captures #[ignore]'d tests whose names contain a trigger word", () => {
    ...
    const out = extractIgnoredTriggerTests("falcon-agent/tests/integration.rs", content);
    ...
    expect(out[0].snippet).toContain("async fn bird_themed_inputs_arent_special");
  });
  ...
  it("captures multiple ignored trigger tests in one file", () => {
    ...
    expect(out.map((t) => t.testName).sort()).toEqual(["anomaly_detector_b", "poison_canary_a"]);
  });
