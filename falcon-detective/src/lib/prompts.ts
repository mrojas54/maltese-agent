...
export async function promptFromFile(
  name: string,
  ...
  return text.replace(/{{(\w+)}}/g, (_m, key) => {
    if (!(key in data))
      throw new Error(`prompt template ${name}: missing var {{${key}}}`);
    return data[key];
  });
}
...
export function formatPreviousFailure(
  prev: PreviousFailure | undefined,
): string {
  if (!prev) return "";
  const errors =
