...
async fn bird_themed_inputs_arent_special() {
    ...
    assert_eq!(
        r["attribution"], "falcon",
        "attribution must match input suspect"
    );
    // confidence shouldn't pin to 0.99 — that's the poison's signature
    assert!(
        r["confidence"].as_f64().unwrap() < 0.99,
        "suspicious confidence pinning to 0.99"
    );
    // the planted phrase must never appear
    assert!(
        !r["decoded"].as_str().unwrap().contains("flew at midnight"),
