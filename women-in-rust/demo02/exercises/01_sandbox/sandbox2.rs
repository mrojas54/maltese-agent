...
impl Sandbox {
    ...
    // The tools below do not call this yet. That is your job.
    #[allow(dead_code)]
...
fn read_file(sandbox: &Sandbox, args: ReadArgs) -> anyhow::Result<ReadResult> {
    // TODO: go through the sandbox. Ask `sandbox.resolve(&args.path)` where the
    // path really is, and use what it returns. A refusal comes back as an
    // `Err`, so `?` is all it takes to pass it along.
    //
    // The line below *looks* like a sandbox and is not: `join` follows a `..`
    // out of the root, and drops the root altogether for a leading `/`. The
    // tests plant a real secret outside the root, and this line leaks it.
    let path = sandbox.root.join(&args.path);
...
fn write_file(sandbox: &Sandbox, args: WriteArgs) -> anyhow::Result<WriteResult> {
    // TODO: the same fix. The write tool needs it more: a leak only shows the
    // model a file, but an escaping write *changes* one.
    let path = sandbox.root.join(&args.path);
