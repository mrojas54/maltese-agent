...
impl Sandbox {
    ...
    fn resolve(&self, rel: impl AsRef<Path>) -> anyhow::Result<PathBuf> {
        ...
        // TODO: refuse a path that can leave the root, judging it by how it is
        // spelled.
        //
        //   - `rel.components()` yields one `std::path::Component` per piece:
        //       Component::ParentDir   a `..`
        //       Component::RootDir     a leading `/`
        //       Component::Prefix(_)   a Windows drive such as `C:`
        //       Component::CurDir      a `.`             (harmless)
        //       Component::Normal(_)   an ordinary name  (harmless)
        //   - Refuse the first three, and let the last two through:
        //       anyhow::bail!(
        //           "path {} escapes sandbox root {}",
        //           rel.display(),
        //           self.root.display()
        //       );
        //     The tests, and later the tool boundary, look for "escapes".
        //   - Do not search the string for "..". A file called `notes..txt` is
        //     fine, and `components()` is what knows the difference.
        //
        // As written it just joins the path onto the root, which is exactly the
        // bug: `join` follows a `..` out, and drops the root for a `/`.
