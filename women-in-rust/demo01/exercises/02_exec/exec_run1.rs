...
impl Demo01 {
    ...
    async fn exec_run(&self, params: Parameters<ExecArgs>) -> Result<Json<ExecResult>, ErrorData> {
        ...
        // TODO: run `cmd` with `args` and return its output.
        //
        //   - Build the command:
        ...
        //           .output()
        //           .await
        //   - `.output()` gives `io::Result<std::process::Output>`. On error,
        //     `ErrorData::internal_error(format!("running {cmd}: {e}"), None)`.
        //   - On success, `out` has `.stdout: Vec<u8>`, `.stderr: Vec<u8>`, and
        //     `.status`. Decode the bytes with
        //     `String::from_utf8_lossy(&out.stdout).into_owned()` and read the
        //     code with `out.status.code().unwrap_or(-1)`.
        let _ = (cmd, args, Stdio::null());
        todo!("run the command and return {{stdout, stderr, exit}}")
    }
}
