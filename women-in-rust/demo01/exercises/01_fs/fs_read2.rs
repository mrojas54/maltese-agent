...
impl Demo01 {
    ...
    async fn fs_read(&self, params: Parameters<ReadArgs>) -> Result<Json<ReadResult>, ErrorData> {
        ...
        // TODO: reject an empty path before reading. Return
        // `ErrorData::invalid_params("path must not be empty", None)` when
        // `path` is empty, so the client sees -32602 and no result.
