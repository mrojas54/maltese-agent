...
//
// Override `get_info`: enable the tools capability, and set the server info
// from `env!("CARGO_PKG_NAME")` and `env!("CARGO_PKG_VERSION")` written in
// *this* file, where they expand to this package's values.
//
...
impl ServerHandler for Demo00 {
    // TODO: override `get_info(&self) -> ServerInfo`. Start from
    // `ServerInfo::new(ServerCapabilities::builder().enable_tools().build())`
    // and chain `.with_server_info(Implementation::new(name, version))`.
}
