...
// path tries to escape its sandbox. Make `greet` reject a blank name with an
// invalid-params error before it does anything else.
//
//     cargo run -- run tool2
...
impl Demo00 {
    ...
    async fn greet(&self, params: Parameters<GreetArgs>) -> Result<Json<GreetResult>, ErrorData> {
        ...
        // TODO: if `name` is blank (empty, or only whitespace), return
        // `Err(ErrorData::invalid_params(..., None))` instead of greeting.
