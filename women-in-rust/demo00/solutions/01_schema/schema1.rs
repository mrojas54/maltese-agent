...
//
// `Deserialize` lets rmcp parse the JSON the model sends into this struct.
// `JsonSchema` lets rmcp describe the struct to the model. The doc comment on
// the field is copied into the schema as its `description`, which is the only
// explanation of `name` the model will ever get.
//
